...
class TestUtilsCamelifyKeys(unittest.TestCase):
    def test_flat_dict(self):
        self.assertEqual(
            camelify_keys({ 'first_name': "Tony", 'last_name': "Stark" }),
            { 'firstName': "Tony", 'lastName': "Stark" }
            )
    def test_nested_dict(self):
        self.assertEqual(
            camelify_keys({ 'org_name': "S.H.I.E.L.D.", 'contact': { 'first_name': "Maria" } }),
            { 'orgName': "S.H.I.E.L.D.", 'contact': { 'firstName': "Maria" } }
            )
    def test_list_of_dicts(self):
        self.assertEqual(
            camelify_keys({ 'assets': [{ 'unique_id': 'MK_VIII', 'status': 'active' }, { 'unique_id': 'MK_XIIc', 'status': 'unknown' }] }),
